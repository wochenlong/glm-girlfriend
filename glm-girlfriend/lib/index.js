...
exports.Config = koishi_1.Schema.object({
    apiAddress: koishi_1.Schema.string().description("后端服务器地址").default(""),
    botname: koishi_1.Schema.string().description("机器人的名字").default("风灵"),
    memoryShortLength: koishi_1.Schema.number()
        .max(16)
        .min(2)
        .description("机器人的短期记忆（位于内存中）长度")
        .default(4),
    randomReplyFrequency: koishi_1.Schema.number()
        .max(10)
        .min(0)
...
function apply(ctx, config) {
    ...
    ctx.middleware(async (session, next) => {
        ...
        if (getReplyCondition(session, config)) {
            ...
            const session_id = `&msg=${encodeURIComponent(userText)}&usrid=1000&source=glm-girlfriend`;
